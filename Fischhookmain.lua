...
local WatermarkConnection = game:GetService('RunService').RenderStepped:Connect(function()
    ...
    Library:SetWatermark(('Fischhook | %s fps | %s ms'):format(
        math.floor(FPS),
...
CreditsGroup:AddLabel('ashysense')
CreditsGroup:AddLabel('ass script lowk')
...
local DiscordButton2 = CreditsGroup:AddButton({
    Text = 'Copy Main tele link',
    Func = function()
        setclipboard('https://t.me/AshysenseRU')
    end,
    DoubleClick = false,
    Tooltip = 'Join our main Tele'
})

CreditsGroup:AddLabel('kubiridzira kwakanyanya')
